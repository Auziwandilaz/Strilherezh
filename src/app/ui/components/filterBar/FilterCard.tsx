...
export const FilterCard = ({
    ...
    return (
        <div className="h-[14rem] w-20% border-lightgray border-r-[1px] border-solid">
            <form key={title} action="/filter" method="post" className="flex flex-col gap-[1.25rem] items-start justify-between h-auto w-[15rem] mx-[3.125rem]">
                    <label>
                        <Typography
                        variant="p"
                        color="black"
                    ...
                    {list.map((content, index) => (
                        <div key={index} className="flex items-center justify-between w-fit h-fit">
                            <input 
                            key={content}
